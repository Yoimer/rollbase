...
var getCalibrationsFromIds = function getCalibrationsFromIds(calibrationIds) {
    ...
    var query = "SELECT " + fields.join(",") + " FROM MDE_Calibration WHERE id IN (" + calibrationIds.join(",") + ")" + "AND status#value = 'Approved'";
...
var humanTimeDelta = function humanTimeDelta(start, end) {
    ...
    /* ------------------------------------------------------------------------------------------------------------- *
     * Get delta date (in +/- Days) for field "Calibration_Counter_Json". It is used by Calibration charts in JSON1. *
     * ------------------------------------------------------------------------------------------------------------- */
    daysExprCnt = Math.ceil(delta / day);
...
var calibrationComponent = function calibrationComponent(id) {
    var calibrationIds = rbv_api.getRelatedIds("R1199850", id);
    ...
    rbv_api.setFieldValue("MDE", id, "Days_Expired_INT", getHtml(data))


    rbv_api.setFieldValue("MDE", id, "Calibration_Counter_Json", daysExprCnt);   // Store in Calibration counter field the days count (JSON1).
};
