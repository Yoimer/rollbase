...
var mdeId = rbv_api.selectQuery("SELECT mdecode_ FROM dim_ApiMde_t1 WHERE mdecode_= '" + mdeName + "'", 1);
//var mdeId = 1;
...
if (rbv_api.isUpdate()) {
    if (mdeId.length > 0) {
        arr["operation_"] = "U";
        rbv_api.updateRecord("dim_ApiMde_t1", mdeId[0][0], arr);
    }

// if creates record
} else if (rbv_api.isCreate()){
    arr["operation_"] = "I";
    rbv_api.createRecord("dim_ApiMde_t1", arr);
}