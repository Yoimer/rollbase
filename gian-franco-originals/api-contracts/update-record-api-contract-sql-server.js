...
var contractCode_SQL_Server = rbv_api.selectQuery("SELECT contractcode_ FROM dim_APIContracts_t WHERE contractcode_ = '" + contractCode + "'", 999);
...
var clientCode_SQL_Server = rbv_api.selectQuery("SELECT clientcode_ FROM dim_ApiCustomer_t WHERE clientname_='" + customerName + "'", 999);
...
if ((contractCode_SQL_Server.length > 0) && (clientCode_SQL_Server.length > 0)) {
    ...
    arr["operation_"] = "U";
    rbv_api.updateRecord("dim_APIContracts_t", contractCode_SQL_Server[0][0], arr);
